...
PalRoute：Palworld 1.0 配种计算器，支持正向/反向查询、可分享状态，以及基于已有 Palbox 物种的配种路线规划 / PalRoute — Palworld 1.0 breeding calculator with forward/reverse lookup, shareable state, and Palbox-based species route planning

https://palroute.online/

PalBreeding.online：Palworld 1.0 免费配种计算器，支持父母组合查子代和目标帕鲁反查全部父母组合；另有可搜索的 299 帕鲁图鉴与公开数据状态页，无需登录 / PalBreeding.online — free Palworld 1.0 breeding calculator with parent-to-child and target-to-all-parent-pairs lookup, plus a searchable 299-Pal directory and public data status page; no login required

https://palbreeding.online/
